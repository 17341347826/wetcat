			...
			<view style="width: 100%;display: inline-block;" :class="{'translateY': index===9}">
				...
				<view class="card inline-block">
					...
					<view style="width: 100%;flex-wrap: wrap;max-height: 968upx;overflow: hidden;" @click="goDetail(item)">
						...
						<text v-if="item.imgs && item.imgs.length > 1 && index !== 9" class="num-photo">共{{item.imgs.length}}张图片</text>
						<view v-if="item.imgs && item.imgs.length > 1 && index !== 9" class="card-num-view" @click="goDetail(item)">
...
<script>
	export default {
		...
		methods: {
				...
				if (this.token) {
					...
					uni.showModal({
						...
						success: function(res) {
							if (res.confirm) {
								uni.redirectTo({
									url: '../../pages/login/login?url=' + '/new/recommend'
								})
							} else if (res.cancel) {
								console.log('用户点击取消');
							}
			...
			gotoVideo() {
				uni.redirectTo({
					url: '../../pages/video/video?url=/new/recommend'
				})
			}
		}
	}
